...
const util = {
    ...
    getIncludePath: function (nameArray, extension) {
        ...
        if (!path) throw '[KEDITOR.util.getIncludePath.fail] The KEDITOR installation path could not be automatically detected. (name: +' + name + ', extension: ' + extension + ')';
    ...
    isWysiwygDiv: function (element) {
        if (element && element.nodeType === 1 && (this.hasClass(element, 'ke-wrapper-wysiwyg') || /^BODY$/i.test(element.nodeName))) return true;
        return false;
    },
