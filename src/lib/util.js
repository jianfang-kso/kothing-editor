...
const util = {
    ...
    getIncludePath: function (nameArray, extension) {
        ...
        path.indexOf(':/') === -1 && path.slice(0, 2) !== '//' && (path = path.indexOf('/') === 0 ? window.location.href.match(/^.*?:\/\/[^\/]*/)[0] + path : window.location.href.match(/^[^\?]*\/(?:)/)[0] + path);

        if (!path) throw Error('[KothingEditor.util.getIncludePath.fail] The KothingEditor installation path could not be automatically detected. (name: +' + path + ', extension: ' + extension + ')');
        ...
        this.getParentElement(node, function (el) {
            ...
            if (finds && !this.isWysiwygDiv(el)) {
                ...
                if (_newOffsets && el.nodeType === 3) {
                    let temp = null; let tempText = null;
    ...
    splitElement: function (baseNode, offset, depth) {
        ...
        let index = 0; let newEl; let children; let temp;
        let next = true;
    ...
    mergeNestedTags: function (element, validation) {
        if (typeof validation === 'string') {
            validation = function (current) { return this.test(current.tagName); }.bind(new this._w.RegExp('^(' + (validation || '.+') + ')$', 'i'));
    ...
    _consistencyCheckOfHTML: function (documentFragment, htmlCheckWhitelistRegExp) {
         ...
         */
        const removeTags = []; const emptyTags = []; const wrongList = []; const withoutFormatCells = [];
    ...
    _setIframeCssTags: function (options) {
        ...
        for (let f = 0, len = linkNames.length, path; f < len; f++) {
            ...
            if (!path || path.length === 0) throw Error('[KothingEditor.constructor.iframe.fail] The KothingEditor CSS files installation path could not be automatically detected. Please set the option property "iframeCSSFileName" before creating editor instances.');
