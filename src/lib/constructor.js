...
export default {
    ...
    init: function (element, options) {
        ...
        if (element.id) top_div.id = 'kothingEditor_' + element.id;
    ...
    _setOptions: function (mergeOptions, context, plugins, originOptions) {
        ...
        const isNewToolbar = !!mergeOptions.toolBarItem || mergeOptions.mode !== originOptions.mode || isNewToolbarContainer;
        const isNewPlugins = !!mergeOptions.plugins;

        const tool_bar = this._createToolBar(document, (isNewToolbar ? mergeOptions.toolBarItem : originOptions.toolBarItem), (isNewPlugins ? mergeOptions.plugins : plugins), mergeOptions);
    ...
    _initOptions: function (element, options) {
        ...
        options.iframeCSSFileName = options.iframe ? typeof options.iframeCSSFileName === 'string' ? [options.iframeCSSFileName] : (options.iframeCSSFileName || ['kothingEditor']) : null;
        ...
        /** Bottom resizing bar */
        options.resizingBar = options.resizingBar === undefined ? (/inline|balloon/i.test(options.mode)) : options.resizingBar;
    ...
    _defaultButtons: function (options) {
        ...
        return {
            ...
            /** plugins - submenu */
            font: ['ke-btn-select ke-btn-tool-font', lang.toolbar.font, 'font', 'submenu', '<span class="txt">' + lang.toolbar.font + '</span>' + icons.arrow_down],
            formatBlock: ['ke-btn-select ke-btn-tool-format', lang.toolbar.formats, 'formatBlock', 'submenu', '<span class="txt">' + lang.toolbar.formats + '</span>' + icons.arrow_down],
            fontSize: ['ke-btn-select ke-btn-tool-size', lang.toolbar.fontSize, 'fontSize', 'submenu', '<span class="txt">' + lang.toolbar.fontSize + '</span>' + icons.arrow_down],
    ...
    _createToolBar: function (doc, toolBarItem, _plugins, options) {
        ...
        if (_buttonTray.children.length === 1) util.removeClass(_buttonTray.firstElementChild, 'ke-btn-module-border');
