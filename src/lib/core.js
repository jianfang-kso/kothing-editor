...
export default function (context, pluginCallButtons, plugins, lang, options, _responsiveButtons) {
    ...
    const core = {
         ...
         * @param {Element|null} _target Plugin target item (This is not necessary if you have a toolbar item when creating the editor)
         */
        ...
        getRange: function () {
            ...
            if (selection.rangeCount > 0) {
                ...
                const sc = selection.anchorNode; const ec = selection.focusNode; const so = selection.anchorOffset; const eo = selection.focusOffset;
                const compareValue = util.compareElements(sc, ec);
                const rightDir = compareValue.ancestor && (compareValue.result === 0 ? so <= eo : compareValue.result > 1);
        ...
        insertNode: function (oNode, afterNode, checkCharCount) {
            ...
            let parentNode; let originAfter = null;
            ...
            for (let i = 0, len = rangeLines.length, line, originParent, depth, before, nextLine, nextList, nested; i < len; i++) {
                ...
                if (util.isList(originParent)) {
                    ...
                    if (i === len - 1 || (nextLine && nextLine.parentNode !== originParent)) {
                        ...
                        let list = originParent.parentNode; let p;
                ...
                if (selectedFormats && lineIndex === -1) {
                    ...
                    if (!reset) {
                        ...
                        if (reset) {
                            ...
                            children = util.getListChildNodes(rangeElement, (current) => current.parentNode === rangeElement);
        ...
        nodeChange: function (appendNode, styleArray, removeNodeArray, strictRemove) {
                    ...
                    while (!util.isFormatElement(sNode) && !util.isWysiwygDiv(sNode)) {
                        for (let i = 0; i < checkAttrs.length; i++) {
                            if (sNode.nodeType === 1) {
                                ...
                                const classReg = /^\./.test(s) ? new RegExp('\\s*' + s.replace(/^\./, '') + '(\\s+|$)', 'ig') : false;
            ...
            let start = {}; let end = {};
            let newNode; let styleRegExp = ''; let classRegExp = ''; let removeNodeRegExp = '';
        ...
        _nodeChange_middleLine: function (element, newInnerNode, validation, isRemoveFormat, isRemoveNode, _removeCheck, _endContainer) {
            ...
            if (!isRemoveNode) {
                ...
                let i = 0; let len = children.length;
        ...
        print: function () {
            ...
            if (options.iframe) {
                const arrts = options.fullPage ? util.getAttributesToString(wDoc.body, ['contenteditable']) : 'class="kothing-editor-editable"';
                ...
                printDocument.write('' +
                    ...
                    linkHTML +
                    '</head>' +
                    '<body class="kothing-editor-editable">' + contentsHTML + '</body>' +
                    '</html>'
                );
            }
        ...
        preview: function () {
            ...
            if (options.iframe) {
                ...
                windowObject.document.write('' +
                    ...
                    linkHTML +
                    '</head>' +
                    '<body class="kothing-editor-editable" style="width:' + w + '; border:1px solid #ccc; margin:10px auto !important; height:auto !important;">' + contentsHTML + '</body>' +
                    '</html>'
                );
            }
        },
        ...
        convertHTMLForCodeView: function (html) {
            let returnHTML = '';
            const RegExp = _w.RegExp;
            const brReg = new RegExp('^(BLOCKQUOTE|PRE|TABLE|THEAD|TBODY|TR|TH|TD|OL|UL|IMG|IFRAME|VIDEO|AUDIO|FIGURE|FIGCAPTION|HR|BR|CANVAS|SELECT)$', 'i');
            ...
            (function recursionFunc(element, indent, lineBR) {
                ...
                for (let i = 0, len = children.length, node, br, nodeRegTest; i < len; i++) {
                    ...
                    returnHTML += (lineBR || (elementRegTest ? '' : br)) + (elementIndent || nodeRegTest ? indent : '') + node.outerHTML.match(RegExp('<' + tag + '[^>]*>', 'i'))[0] + br;
        ...
        _init: function (reload, _initHTML) {
            ...
            this._disallowedTextTagsRegExp = disallowTextTags.length === 0 ? null : new RegExp('(<\\/?)(' + disallowTextTags.join('|') + ')\\b\\s*(?:[^>^<]+)?\\s*(?=>)', 'gi');
            ...
            this._htmlCheckWhitelistRegExp = new RegExp('^(' + options._editorTagsWhitelist.replace('|//', '') + ')$', 'i');
            ...
            if (_attr) {
                for (const k in _attr) {
                    ...
                    if (k === 'all') {
                        ...
                        tagsAttr[k] = new RegExp('((?:' + _attr[k] + '|' + defaultAttr + ')\s*=\s*"[^"]*")', 'ig');
                    }
                }
            }

            this._attributesWhitelistRegExp = new RegExp('((?:' + allAttr + defaultAttr + ')\s*=\s*"[^"]*")', 'ig');
            ...
            this._fileManager.pluginRegExp = new RegExp('^(' + (filePluginRegExp.length === 0 ? 'undefined' : filePluginRegExp.join('|')) + ')$', 'i');
    ...
    const event = {
        ...
        onKeyDown_wysiwyg: function (e) {
            ...
            const cells = [];
            let lines = [];
            let fc = util.isListCell(selectedFormats[0]); let lc = util.isListCell(selectedFormats[selectedFormats.length - 1]);
            ...
            switch (keyCode) {
                ...
                case 13: /** enter key */
                    ...
                    if (!shift && freeFormatEl) {
                        ...
                        const children = selectionNode.childNodes; const offset = wSelection.focusOffset; const prev = selectionNode.previousElementSibling; const next = selectionNode.nextSibling;
                        ...
                        if (selectionFormat) {
                            ...
                            const brPrev = br.previousSibling; const brNext = br.nextSibling;
        ...
        onKeyUp_wysiwyg: function (e) {
            ...
            if (textKey && selectionNode.nodeType === 3 && util.zeroWidthRegExp.test(selectionNode.textContent) && !(e.isComposing !== undefined ? e.isComposing : event._IEisComposing)) {
                let so = range.startOffset; let eo = range.endOffset;
    ...
    const functions = {
        ...
        */
         ...
         * @param {Array} toolBarItem Toolbar item
         */
        setToolbarButtons: function (toolBarItem) {
            core.submenuOff();
            core.containerOff();

            const newToolbar = _Constructor._createToolBar(_d, toolBarItem, core.plugins, options);
            ...
            const mergeOptions = [options, _options].reduce(function (init, option) {
                for (const key in option) {
                    ...
                    if (key === 'plugins' && option[key] && init[key]) {
                        let i = init[key]; let o = option[key];
