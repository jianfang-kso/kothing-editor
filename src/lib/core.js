...
export default function(context, pluginCallButtons, plugins, lang, _options) {
  ...
  const core = {
    ...
    /**
     * @description Elements and user options parameters of the kothing-editor
     */
    context: context,
    ...
    _imageUpload: function(
      ...
      if (typeof userFunction.onImageUpload === "function")
        userFunction.onImageUpload(
          targetImgElement,
          index * 1,
          state,
          imageInfo,
    ...
    _imageUploadError: function(errorMessage, result) {
      if (typeof userFunction.onImageUploadError === "function")
        return userFunction.onImageUploadError(errorMessage, result);
      return true;
    },
    ...
    submenuOn: function(element) {
      ...
      const overLeft =
        this.context.element.toolbar.offsetWidth -
        (element.parentElement.offsetLeft + this.submenu.offsetWidth);
    ...
    getSelectionNode: function() {
      if (
        ...
        util.isWysiwygDiv(this._variable._selectionNode)
      )
        this._editorRange();
      return (
        this._variable._selectionNode || context.element.wysiwyg.firstChild
      );
    },
    ...
    _createDefaultRange: function() {
      const range = this._wd.createRange();
      if (!context.element.wysiwyg.firstChild)
    ...
    appendFormatTag: function(element, formatNodeName) {
      ...
      if (util.isCell(formatEl))
        formatEl.insertBefore(oFormat, element.nextElementSibling);
      else
        formatEl.parentNode.insertBefore(oFormat, formatEl.nextElementSibling);
    ...
    insertComponent: function(element) {
      ...
      if (util.isListCell(formatEl)) {
        if (/^HR$/i.test(element.nodeName)) {
          ...
          this.insertNode(
            element,
            selectionNode === formatEl ? null : selectionNode
          );
        ...
        if (range.collapsed) {
          if (commonCon.nodeType === 3) {
            ...
            if (
              ...
              util.isBreak(parentNode.lastChild)
            ) {
    ...
    applyRangeFormatElement: function(rangeElement) {
      ...
      if (util.isRangeFormatElement(last) || util.isFormatElement(last)) {
        ...
        standTag =
          util.getRangeFormatElement(last) || util.getFormatElement(last);
      }
      ...
      for (
        let i = 0, len = rangeLines.length, line, originParent, depth, before;
        i < len;
        i++
      ) {
        line = rangeLines[i];
        ...
        if (util.isList(originParent)) {
          if (listParent === null)
            listParent = util.createElement(originParent.nodeName);
          ...
          if (
            i === len - 1 ||
            !this.util.getParentElement(rangeLines[i + 1], function(current) {
              return current === originParent;
            })
          ) {
      ...
      for (let i = 0, len = children.length, insNode; i < len; i++) {
        ...
        if (selectedFormats && selectedFormats.indexOf(insNode) === -1) {
          ...
          if (!newList && util.isListCell(insNode)) {
            ...
            insNode = util.isCell(rangeElement.parentNode)
    ...
    nodeChange: function(appendNode, styleArray, removeNodeArray) {
          ...
          for (let i = 0; i < styleArray.length; i++) {
            while (!util.isFormatElement(sNode) && !util.isWysiwygDiv(sNode)) {
              if (
                sNode.nodeType === 1 &&
                (isRemoveNode
                  ? sNode.style[styleArray[i]]
                  : sNode.style[styleArray[i]] ===
                    appendNode.style[styleArray[i]])
              ) {
                checkCnt++;
              }
      ...
      if (!util.getFormatElement(endCon)) {
        endCon = util.getChildElement(lineNodes[lineNodes.length - 1], function(
          current
        ) {
          return current.nodeType === 3;
        });
      ...
      const oneLine =
        util.getFormatElement(startCon) === util.getFormatElement(endCon);
      ...
      switch (command) {
        ...
        default:
          ...
          if (
            ...
            command === "SUP" &&
            util.hasClass(this.commandMap.SUB, "active")
          ) {
            this.nodeChange(null, null, ["SUB"]);
          }
          this.nodeChange(on ? null : this.util.createElement(command), null, [
            command,
          ]);
          this.focus();
      }
    ...
    indent: function(command) {
      ...
      for (let i = 0, len = rangeLines.length; i < len; i++) {
        p = rangeLines[i];
        margin = /\d+/.test(p.style.marginLeft)
    ...
    toggleDisplayBlocks: function() {
      util.toggleClass(context.element.wysiwyg, "ke-show-block");
    },
    ...
    toggleCodeView: function() {
      ...
      if (!wysiwygActive) {
        ...
        if (context.option.fullPage) {
          const parseDocument = new this._w.DOMParser().parseFromString(
            code_html,
            "text/html"
          );
          ...
          this._wd.body.innerHTML = util.convertContentsForEditor(
          ...
          context.element.wysiwyg.innerHTML =
            code_html.length > 0
              ? util.convertContentsForEditor(code_html)
              : "<p><br></p>";
        }
        ...
        if (context.option.fullPage) {
          ...
          codeValue =
            ...
            "\n" +
            util.convertHTMLForCodeView(context.element.wysiwyg) +
            "</body>\n</html>";
        } else {
        ...
        if (context.option.codeMirrorEditor)
          context.option.codeMirrorEditor.refresh();
    ...
    toggleFullScreen: function(element) {
      ...
      if (!this._variable.isFullScreen) {
        ...
        wysiwyg.style.cssText = wysiwyg.style.cssText.match(
          /\s?display(\s+)?:(\s+)?[a-zA-Z]+;/
        )[0];
        ...
        this._variable.innerHeight_fullScreen =
          _w.innerHeight - toolbar.offsetHeight;
        editorArea.style.height = this._variable.innerHeight_fullScreen + "px";

        util.removeClass(element.firstElementChild, "ke-icon-expansion");
        util.addClass(element.firstElementChild, "ke-icon-reduction");
      } else {
        ...
        util.removeClass(element.firstElementChild, "ke-icon-reduction");
        util.addClass(element.firstElementChild, "ke-icon-expansion");
      }
    },
    ...
    print: function() {
      ...
      contents.className = "kothing-editor-editable";
      ...
      try {
        ...
        // IE or Edge
        if (
          _w.navigator.userAgent.indexOf("MSIE") !== -1 ||
          !!_d.documentMode ||
          !!_w.StyleMedia
        ) {
          try {
      ...
      if (maxCharCount > 0) {
        ...
        if (count > maxCharCount) {
          ...
          core.getSelectionNode().textContent =
            text.slice(0, range.endOffset - 1) +
        ...
        if (over) {
          if (blink && !util.hasClass(charCounter, "ke-blink")) {
            util.addClass(charCounter, "ke-blink");
            _w.setTimeout(function() {
              util.removeClass(charCounter, "ke-blink");
            }, 600);
          }
    ...
    _checkComponents: function() {
      if (this.plugins.image) {
        if (!this.initPlugins.image)
          this.callPlugin(
            "image",
            this.plugins.image.checkImagesInfo.bind(this)
          );
        else this.plugins.image.checkImagesInfo.call(this);
      }

      if (this.plugins.video) {
        if (!this.initPlugins.video)
          this.callPlugin(
            "video",
            this.plugins.video.checkVideosInfo.bind(this)
          );
    ...
    _getCodeView: function() {
      return context.option.codeMirrorEditor
        ? context.option.codeMirrorEditor.getDoc().getValue()
     ...
     */
    _init: function() {
      core._ww = context.option.iframe
      ...
      core.codeViewDisabledButtons = context.element.toolbar.querySelectorAll(
        '.ke-toolbar button:not([class~="code-view-enabled"])'
      );
   ...
   */
  const event = {
    _directionKeyKeyCode: new _w.RegExp(
      "^(8|13|32|46|33|34|35|36|37|38|39|40|46|98|100|102|104)$"
    ),
    _historyIgnoreKeycode: new _w.RegExp(
      "^(9|1[6-8]|20|3[3-9]|40|45|11[2-9]|12[0-3]|144|145)$"
    ),
    ...
    _findButtonEffectTag: function() {
      ...
      for (
        let selectionParent = core.getSelectionNode();
        !util.isWysiwygDiv(selectionParent);
        selectionParent = selectionParent.parentNode
      ) {
        if (!selectionParent) break;
        if (selectionParent.nodeType !== 1 || util.isBreak(selectionParent))
          continue;
        ...
        if (util.isFormatElement(selectionParent)) {
          ...
          if (findAlign && textAlign && commandMap.ALIGN) {
            ...
            commandMap.ALIGN.className = "ke-icon-align-" + textAlign;
          ...
          if (
            findOutdent &&
            selectionParent.style.marginLeft &&
            (selectionParent.style.marginLeft.match(/\d+/) || [0])[0] * 1 > 0 &&
            commandMap.OUTDENT
          ) {
        ...
        if (findList && util.isList(nodeName) && commandMap.LI) {
          ...
          if (/UL/i.test(nodeName)) {
            util.removeClass(commandMap.LI_ICON, "ke-icon-list-number");
            util.addClass(commandMap.LI_ICON, "ke-icon-list-bullets");
          } else {
            util.removeClass(commandMap.LI_ICON, "ke-icon-list-bullets");
            util.addClass(commandMap.LI_ICON, "ke-icon-list-number");
          }
          findList = false;
        }
        ...
        if (
          findFont &&
          selectionParent.style.fontFamily.length > 0 &&
          commandMap.FONT
        ) {
          commandMapNodes.push("FONT");
          const selectFont = (
            selectionParent.style.fontFamily ||
            selectionParent.face ||
            lang.toolbar.font
          ).replace(/["']/g, "");
        ...
        if (
          findSize &&
          selectionParent.style.fontSize.length > 0 &&
          commandMap.SIZE
        ) {
        ...
        if (
          findA &&
          /^A$/.test(nodeName) &&
          selectionParent.getAttribute("data-image-link") === null &&
          core.plugins.link
        ) {
          if (
            !context.link ||
            core.controllerArray[0] !== context.link.linkBtn
          ) {
            core.callPlugin("link", function() {
              core.plugins.link.call_controller_linkButton.call(
                core,
                selectionParent
              );
            });
      ...
      for (const key in commandMap) {
        ...
        if (commandMap.FONT && /^FONT$/i.test(key)) {
          ...
          commandMap.ALIGN.className = "ke-icon-align-left";
      ...
      if (context.option.showPathLabel)
      ...
      if (display) {
        if (
          ...
          (target.nextElementSibling === null ||
            target !== core.submenuActiveButton)
        ) {
    ...
    onClick_wysiwyg: function(e) {
      ...
      if (context.element.wysiwyg.getAttribute("contenteditable") === "false")
        return;
      e.stopPropagation();
      ...
      if (/^FIGURE$/i.test(targetElement.nodeName)) {
        ...
        if (imageComponent) {
          ...
          core.callPlugin("image", function() {
            const size = core.plugins.resizing.call_controller_resize.call(
              core,
              imageComponent,
              "image"
            );
            ...
            if (!util.getParentElement(imageComponent, ".ke-image-container")) {
      ...
      if (
        ...
        core.execCommand(
          "formatBlock",
          false,
          util.isRangeFormatElement(rangeEl) ? "DIV" : "P"
        );
        core.focus();
      }
    ...
    _showToolbarInline: function() {
      ...
      core._inlineToolbarAttr.width = toolbar.style.width =
        context.option.toolbarWidth;
      core._inlineToolbarAttr.top = toolbar.style.top =
        -1 - toolbar.offsetHeight + "px";

      if (typeof userFunction.showInline === "function")
        userFunction.showInline(toolbar, context);
    ...
    onKeyDown_wysiwyg: function(e) {
      ...
      switch (keyCode) {
        case 8 /** backspace key */:
          ...
          if (
            util.isWysiwygDiv(selectionNode.parentNode) &&
            !selectionNode.previousSibling &&
            util.isFormatElement(selectionNode) &&
            !util.isListCell(selectionNode)
          ) {
            e.preventDefault();
            e.stopPropagation();
        ...
        case 46 /** delete key */:
          ...
          if (
            ...
            if (util.isComponent(nextEl)) {
              ...
              if (
                util.hasClass(nextEl, "ke-component") ||
                /^IMG$/i.test(nextEl.nodeName)
              ) {
                e.stopPropagation();
                if (
                  util.hasClass(nextEl, "ke-image-container") ||
                  /^IMG$/i.test(nextEl.nodeName)
                ) {
                  nextEl = /^IMG$/i.test(nextEl.nodeName)
                    ? nextEl
                    : nextEl.querySelector("img");
                  core.callPlugin("image", function() {
                    const size = core.plugins.resizing.call_controller_resize.call(
                      core,
                      nextEl,
                    ...
                    if (!util.getParentElement(nextEl, ".ke-component")) {
                      core.plugins.image.openModify.call(core, true);
                      core.plugins.image.update_image.call(core, true, true);
                    }
                  });
                } else if (util.hasClass(nextEl, "ke-video-container")) {
                  e.stopPropagation();
                  core.callPlugin("video", function() {
        ...
        case 9 /** tab key */:
          ...
          if (currentNode && util.isCell(currentNode)) {
            ...
            let idx = shift
              ? util.prevIdx(cells, currentNode)
              : util.nextIdx(cells, currentNode);
          ...
          if (!shift) {
            const tabText = util.createTextNode(
              new Array(core._variable.tabSize + 1).join("\u00A0")
            );
            if (lines.length === 1) {
              core.insertNode(tabText);
              core.setRange(
                tabText,
                core._variable.tabSize,
                tabText,
                core._variable.tabSize
              );
            } else {
          ...
          if (
            ...
            if (
              ...
              const newEl = core.appendFormatTag(
                rangeEl,
                util.isCell(rangeEl.parentNode)
                  ? "DIV"
          ...
          if (resizingName) {
            ...
            if (util.isListCell(container.parentNode)) {
              ...
              newEl = util.createElement(
                util.isFormatElement(sibling) ? sibling.nodeName : "P"
              );
              newEl.innerHTML = "<br>";
            }
            ...
            core.callPlugin(resizingName, function() {
              ...
              core.plugins[resizingName].onModifyMode.call(
                core,
                compContext._element,
                size
              );
    ...
    onScroll_window: function() {
      ...
      const y =
        (this.scrollY || _d.documentElement.scrollTop) +
        context.option.stickyToolbar;
      const editorTop =
        event._getStickyOffsetTop() -
    ...
    onPaste_wysiwyg: function(e) {
      ...
      const maxCharCount = core._charCount(
        clipboardData.getData("text/plain").length,
        true
      );
      const cleanData = util.cleanHTML(clipboardData.getData("text/html"));

      if (
        typeof userFunction.onPaste === "function" &&
        !userFunction.onPaste(e, cleanData, maxCharCount)
      ) {
        e.preventDefault();
        e.stopPropagation();
        return false;
    ...
    onDrop_wysiwyg: function(e) {
      ...
      if (files.length > 0 && core.plugins.image) {
        ...
        !core._charCount(dataTransfer.getData("text/plain").length, true)
      ) {
        e.preventDefault();
        e.stopPropagation();
        return false;
  ...
  const userFunction = {
    ...
    setOptions: function(options) {
      ...
      const mergeOptions = [context.option, options].reduce(function(
        init,
        option
      ) {
    ...
    /**
     * @description Gets the kothing-editor's context object. Contains settings, plugins, and cached element objects
     * @returns {Object}
     */
    getContext: function() {
      return context;
    ...
    /**
     * @description Gets the contents of the kothing-editor
     * @returns {String}
     */
    getContents: function() {
    ...
    /**
     * @description Change the contents of the kothing-editor
    ...
    /**
     * @description Add contents to the kothing-editor
     * @param {String} contents Contents to Input
     */
    appendContents: function(contents) {
      if (core._variable.wysiwygActive) {
        context.element.wysiwyg.innerHTML += util.convertContentsForEditor(
          contents
        );
      } else {
        core._setCodeView(
          core._getCodeView() + util.convertHTMLForCodeView(contents)
        );
      }
    ...
    /**
     * @description Disable the kothing-editor
     */
    disabled: function() {
    ...
    /**
     * @description Enabled the kothing-editor
     */
    enabled: function() {
    ...
    /**
     * @description Show the kothing-editor
     */
    show: function() {
      const topAreaStyle = context.element.topArea.style;
      if (topAreaStyle.display === "none")
        topAreaStyle.display = context.option.display;
    },

    /**
     * @description Hide the kothing-editor
     */
    hide: function() {
    ...
    /**
     * @description Destroy the kothing-editor
     */
    destroy: function() {
      ...
      _w.Object.keys(core).forEach(function(key) {
        delete core[key];
      });
      _w.Object.keys(event).forEach(function(key) {
        delete event[key];
      });
      _w.Object.keys(context).forEach(function(key) {
        delete context[key];
      });
      _w.Object.keys(pluginCallButtons).forEach(function(key) {
        delete pluginCallButtons[key];
      });

      /** remove user object */
      _w.Object.keys(this).forEach(
        function(key) {
          delete this[key];
        }.bind(this)
      );
