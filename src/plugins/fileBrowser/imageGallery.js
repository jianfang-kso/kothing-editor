...
export default {
    ...
    drawItems: function (item) {
        ...
        return  '<div class="ke-file-item-img"><img src="' + item.src + '" alt="' + (item.alt || srcName) + '" data-command="pick">' +
                    '<div class="ke-file-img-name ke-file-name-back"></div>' +
                    '<div class="ke-file-img-name __ke__img_name">' + (item.name || srcName) + '</div>' +
                '</div>';
    },

    setImage: function (target) {
        this.callPlugin('image', function () {
            const file = {name: target.parentNode.querySelector('.__ke__img_name').textContent, size: 0};
