...
(function (global, factory) {
    ...
    const fileBrowser = {
        ...
        add: function (core) {
            ...
            this._loading = browser_div.querySelector('.ke-loading-box');
        ...
        set_browser: function (core) {
            ...
            return '<div class="ke-file-browser-content">' +
                '<div class="ke-file-browser-header">' +
                '<button type="button" data-command="close" class="ke-btn ke-file-browser-close" class="close" aria-label="Close" title="' + lang.dialogBox.close + '">' +
                core.icons.cancel +
                '</button>' +
            ...
            if (xmlHttp.readyState === 4) {
                ...
                if (xmlHttp.status === 200) {
                    ...
                    if (xmlHttp.status !== 0) {
                        ...
                        const err = '[KothingEditor.fileBrowser.get.serverException] status: ' + xmlHttp.status + ', response: ' + (res.errorMessage || xmlHttp.responseText);
                        throw Error(err);
                    }
                }
            }
