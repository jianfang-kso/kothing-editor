...
(function(global, factory) {
  ...
  const resizing = {
    ...
    add: function(core) {
      ...
      let resize_handles = (context.resizing.resizeHandles = context.resizing.resizeDot.querySelectorAll(
        'span'
      ));
      ...
      if (
        ...
        if (xy === 'x') {
          contextPlugin.inputY.value =
            this.util.getNumber(
              contextPlugin._ratioY * this.util.getNumber(contextPlugin.inputX.value, dec),
              dec
            ) + yUnit;
        } else {
          contextPlugin.inputX.value =
            this.util.getNumber(
              contextPlugin._ratioX * this.util.getNumber(contextPlugin.inputY.value, dec),
              dec
            ) + xUnit;
        }
      }
    ...
    call_controller_resize: function(targetElement, plugin) {
      ...
      const resizeDotShow =
        contextPlugin._resizing && !contextPlugin._resizeDotHide && !contextPlugin._onlyPercentage
          ? 'flex'
          : 'none';
      ...
      if (contextPlugin._resizing) {
        ...
        rotations[0].style.display = rotations[1].style.display = contextPlugin._rotation
          ? ''
          : 'none';
      }
      ...
      if (this.currentControllerName !== plugin) {
        ...
        this.controllersOn(
          contextResizing.resizeContainer,
      ...
      const overLeft =
        ...
        l -
        contextResizing.resizeButton.offsetWidth;
    ...
    onClick_resizeButton: function(e) {
      ...
      switch (command) {
        ...
        case 'caption':
          ...
          if (caption) {
            const captionText = this.util.getChildElement(currentContext._caption, function(
              current
            ) {
    ...
    setTransformSize: function(element, width, height) {
      ...
      if (percentage && !isVertical) {
        ...
        if (percentage[0] === 'auto' && percentage[1] === 'auto') {
          ...
          this.plugins[this.context.resizing._resize_plugin].setPercentSize.call(
            this,
            percentage[0],
            percentage[1]
          );
    ...
    setCaptionPosition: function(element) {
      ...
      if (figcaption) {
        figcaption.style.marginTop =
          (this.context.resizing._rotateVertical ? element.offsetWidth - element.offsetHeight : 0) +
          'px';
      }
    },
    ...
    cancel_controller_resize: function(direction) {
      ...
      let w = this._w.Math.round(
        isVertical ? this.context.resizing._resize_h : this.context.resizing._resize_w
      );
      let h = this._w.Math.round(
        isVertical ? this.context.resizing._resize_w : this.context.resizing._resize_h
      );
