        ...
        let list = '<div class="ke-list-inner">';
            for (let i = 0, len = colorList.length; i < len; i++) {
                ...
                if (typeof colorList[i] === 'object') {
                    list += '<div class="ke-selector-color">' + makeColor(colorList[i]) + '</div>';
                }
            }
            list += '' +
            '<form class="ke-submenu-form-group">' +
            '   <input type="text" maxlength="7" class="_se_color_picker_input" />' +
            '   <button type="submit" class="ke-btn-primary ke-tooltip _se_color_picker_submit">' +
            '       <i class="ke-icon-checked"></i>' +
            '       <span class="ke-tooltip-inner"><span class="ke-tooltip-text">' + lang.dialogBox.submitButton + '</span></span>' +
            '   </button>' +
            '   <button type="button" class="ke-btn ke-tooltip _se_color_picker_remove">' +
            '       <i class="ke-icon-erase"></i>' +
            '       <span class="ke-tooltip-inner"><span class="ke-tooltip-text">' + lang.toolbar.removeFormat + '</span></span>' +
            '   </button>' +
            '</form>' +
            '</div>';
