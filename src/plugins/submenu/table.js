...
export default {
  ...
  init: function() {
    ...
    if (tablePlugin._selectedTable) {
      const selectedCells = tablePlugin._selectedTable.querySelectorAll('.ke-table-selected-cell');
  ...
  call_controller_tableEdit: function(tdElement) {
    ...
    contextTable._maxWidth =
      ...
      (!tableElement.style.width && !this.util.hasClass(tableElement, 'ke-table-size-auto'));
  ...
  setCellInfo: function(tdElement, reset) {
    ...
    const table = (contextTable._element =
      this.plugins.table._selectedTable || this.util.getParentElement(tdElement, 'TABLE'));
    ...
    if (isRow) {
      ...
      if (/^THEAD$/i.test(tableAttr.nodeName)) {
        if (option === 'up') {
          ...
          table.innerHTML +=
            '<tbody><tr>' +
            tablePlugin.createCells.call(this, 'td', contextTable._logical_cellCnt, false) +
            '</tr></tbody>';
          return;
        }
      }
  ...
  editRow: function(option, positionResetElement) {
    ...
    const rowIndex =
      remove || up ? originRowIndex : originRowIndex + contextTable._current_rowSpan + 1;
      ...
      for (let c = 0, cell, cLen = cells.length, rs, cs, removeIndex; c < cLen; c++) {
        ...
        if (!remove) {
          ...
          if (removeIndex >= insertIndex && removeIndex + cs <= insertIndex + colSpan) {
            ...
            rs > 0 &&
            (removeIndex < insertIndex || removeIndex + cs > insertIndex + colSpan)
          ) {
            removeSpanArr.push({
              cell: cell,
              i: i,
  ...
  _toggleEditor: function(enabled) {
    ...
    if (enabled) this.util.removeClass(this.context.element.wysiwyg, 'ke-disabled');
    else this.util.addClass(this.context.element.wysiwyg, 'ke-disabled');
  },
