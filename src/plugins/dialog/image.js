...
export default {
  ...
  add: function(core) {
    ...
    context.image.imgInputFile = context.image.focusElement = image_dialog.querySelector(
      "._se_image_file"
    );
    ...
    context.image.imgLinkNewWindowCheck = image_dialog.querySelector(
      "._se_image_link_check"
    );
    context.image.captionCheckEl = image_dialog.querySelector(
      "._se_image_check_caption"
    );
    ...
    if (context.option.imageResizing) {
      context.image.proportion = image_dialog.querySelector(
        "._se_image_check_proportion"
      );
      ...
      context.image.imageX.addEventListener(
        "change",
        this.setInputSize.bind(core, "x")
      );
      context.image.imageY.addEventListener(
        "change",
  ...
  setDialog: function() {
    ...
    let html =
      ...
      '   <button type="button" data-command="close" class="close" aria-label="Close" title="' +
    ...
    if (option.imageFileInput) {
      html +=
        ...
        "</label>" +
        '       <input class="ke-input-form _se_image_file" type="file" accept="image/*" multiple="multiple" />' +
        "   </div>";
    }
    ...
    if (option.imageUrlInput) {
      html +=
        ...
        "</label>" +
        '       <input class="ke-input-form _se_image_url" type="text" />' +
        "   </div>";
    }
    ...
    html +=
      ...
      '</label><input class="ke-input-form _se_image_alt" type="text" />' +
      "           </div>";
    ...
    if (option.imageResizing) {
      html +=
        ...
        "</label></div>" +
        '           <input class="ke-input-control _se_image_size_x" type="number" min="1" ' +
        (option.imageWidth === "auto" ? "disabled" : "") +
        ' /><label class="ke-dialog-size-x">x</label><input class="ke-input-control _se_image_size_y" type="number" min="1" disabled />' +
        '           <label><input type="checkbox" class="ke-dialog-btn-check _se_image_check_proportion" checked disabled/>&nbsp;' +
        lang.dialogBox.proportion +
        "</label>" +
        '           <button type="button" title="' +
        lang.dialogBox.revertButton +
        '" class="ke-btn ke-dialog-btn-revert" style="float: right;"><i class="ke-icon-revert"></i></button>' +
        "       </div>";
    }
    ...
    html +=
      ...
      '               <label><input type="checkbox" class="ke-dialog-btn-check _se_image_check_caption" />&nbsp;' +
      ...
      '</label><input class="ke-input-form _se_image_link" type="text" />' +
      "           </div>" +
      '           <label><input type="checkbox" class="_se_image_link_check"/>&nbsp;' +
      ...
      "</label>" +
      "       </div>" +
      '       <button type="button" data-command="close" class="ke-btn-cancel" aria-label="Close" title="' +
      lang.dialogBox.close +
      '"><span data-command="close">' +
      lang.dialogBox.close +
      "</span></button>" +
  ...
  openTab: function(e) {
    ...
    const targetElement =
      e === "init" ? modal.querySelector("._se_tab_link") : e.target;
  ...
  submitAction: function(fileList) {
    if (fileList.length > 0) {
      ...
      if (typeof imageUploadUrl === "string" && imageUploadUrl.length > 0) {
        ...
        this.context.image._xmlHttp.onreadystatechange = this.plugins.image.callBack_imgUpload.bind(
          this,
          this.context.image._linkValue,
          this.context.image.imgLinkNewWindowCheck.checked,
          this.context.image.imageX.value,
          this.context.image._align,
  ...
  onRender_imgInput: function() {
    try {
      this.plugins.image.submitAction.call(
        this,
      ...
      this.closeLoading();
      throw Error(
        '[KothingEditor.imageUpload.fail] cause : "' + e.message + '"'
      );
    }
  },
  ...
  setup_reader: function(
    ...
    reader.onload = function(update, updateElement, file) {
      try {
        ...
        this.closeLoading();
        throw Error(
          '[KothingEditor.imageFileRendering.fail] cause : "' + e.message + '"'
        );
      }
    }.bind(
      this,
  ...
  callBack_imgUpload: function(
    ...
    if (this.context.image._xmlHttp.readyState === 4) {
      ...
      // error
        ...
        this.closeLoading();
        throw Error(
          "[KothingEditor.imageUpload.fail] status: " +
            this.context.image._xmlHttp.status +
            ", responseURL: " +
            this.context.image._xmlHttp.responseURL
        );
      }
    }
  },
    ...
    try {
      ...
      if (this.context.dialog.updateModal)
        ...
        this.plugins.image.create_image.call(
          ...
          this.context.image._linkValue,
          this.context.image.imgLinkNewWindowCheck.checked,
          this.context.image.imageX.value + "px",
          this.context.image._align,
          file
        );
    } catch (e) {
      throw Error(
        '[KothingEditor.imageURLRendering.fail] cause : "' + e.message + '"'
      );
    } finally {
      this.closeLoading();
    }
  ...
  setInputSize: function(xy) {
    ...
    if (contextImage.proportion.checked) {
      if (xy === "x") {
        contextImage.imageY.value = Math.round(
          (contextImage._element_h / contextImage._element_w) *
            contextImage.imageX.value
        );
      } else {
        contextImage.imageX.value = Math.round(
          (contextImage._element_w / contextImage._element_h) *
  ...
  submit: function(e) {
    ...
    try {
      ...
      if (
        ...
        contextImage.imgUrlFile.value.trim().length > 0
      ) {
  ...
  setImagesInfo: function(img, file) {
      ...
      this._w.setTimeout(
        function() {
          this.plugins.image.onModifyMode.call(
            this,
            img,
            this.plugins.resizing.call_controller_resize.call(
              this,
              img,
              "image"
            )
    ...
    this._imageUpload(
      ...
      state,
      info,
      --this.context.image._uploadFileLength < 0
        ? 0
        : this.context.image._uploadFileLength
    );
  },
  ...
  checkImagesInfo: function() {
    ...
    for (let i = 0, len = images.length, img; i < len; i++) {
      img = images[i];
      if (!this.util.getParentElement(img, ".ke-image-container")) {
        ...
        infoIndex.indexOf(img.getAttribute("data-index") * 1) < 0
      ) {
        currentImages.push(this._variable._imageIndex);
  ...
  create_image: function(src, linkValue, linkNewWindow, width, align, file) {
    ...
    oImg.addEventListener(
      "load",
      this.plugins.image._onload_image.bind(this, oImg, file)
    );

    oImg.src = src;
    oImg.setAttribute("data-align", align);
    oImg.alt = contextImage._altText;
    oImg = this.plugins.image.onRender_link.call(
      this,
      oImg,
      linkValue,
      linkNewWindow
    ...
    const container = this.plugins.resizing.set_container.call(
      this,
      cover,
      "ke-image-container"
    );
  ...
  update_image: function(init, openController) {
    ...
    const changeSize =
      contextImage.imageX.value * 1 !== imageEl.offsetWidth ||
      contextImage.imageY.value * 1 !== imageEl.offsetHeight;
    ...
    if (container === null) {
      ...
      isNewContainer = true;
      container = this.plugins.resizing.set_container.call(
        this,
        cover,
        "ke-image-container"
      );
    ...
    if (contextImage._resizing) {
      ...
      if (changeSize)
        this.plugins.image.setSize.call(
          this,
          contextImage.imageX.value,
          contextImage.imageY.value
        );
    ...
    if (
      ...
      (/\d+/.test(imageEl.style.height) ||
        (contextImage._resizing && changeSize) ||
    ...
    if (init) {
      this.plugins.image.setImagesInfo.call(this, imageEl, {
        name:
          imageEl.getAttribute("data-file-name") ||
          imageEl.src.split("/").pop(),
    ...
    if (openController) {
      ...
      const size = this.plugins.resizing.call_controller_resize.call(
        this,
        imageEl,
        "image"
      );
  ...
  update_src: function(src, element, file) {
    element.src = src;
    this._w.setTimeout(
      this.plugins.image.setImagesInfo.bind(this, element, file)
    );
  },
  ...
  sizeRevert: function() {
    ...
    if (contextImage._origin_w) {
      contextImage.imageX.value = contextImage._element_w =
        contextImage._origin_w;
      contextImage.imageY.value = contextImage._element_h =
        contextImage._origin_h;
    }
  },
  ...
  openModify: function(notOpen) {
    ...
    if (contextImage._resizing) {
      contextImage.proportion.checked = contextImage._proportionChecked =
        contextImage._element.getAttribute("data-proportion") !== "false";
  ...
  on: function(update) {
    if (!update) {
      const contextImage = this.context.image;
      contextImage.imageX.value = contextImage._origin_w = contextImage._defaultAuto
        ? ""
  ...
  setAutoSize: function() {
    ...
    contextImage._element.style.width = originSize[0]
      ? originSize[0] + "px"
      : "100%";
  ...
  cancelPercentAttr: function() {
    ...
    this.util.addClass(
      contextImage._container,
      "__se__float-" + contextImage._align
    );
  },
  ...
  destroy: function(element) {
    ...
    const imageContainer =
      this.util.getParentElement(imageEl, ".ke-image-container") || imageEl;
  ...
  init: function() {
    ...
    if (contextImage._resizing) {
      ...
      contextImage.imageX.value = contextImage._defaultAuto
        ? ""
        : this.context.option.imageWidth;
      contextImage.imageY.value = "";
