...
export default {
    ...
    add: function (core) {
        ...
        if (contextAudio.audioInputFile) audio_dialog.querySelector('.ke-dialog-files-edge-button').addEventListener('click', this._removeSelectedFiles.bind(contextAudio.audioInputFile, contextAudio.audioUrlFile, contextAudio.preview));
            ...
            '<form method="post" enctype="multipart/form-data">' +
                '<div class="ke-dialog-header">' +
                    '<button type="button" data-command="close" class="ke-btn ke-dialog-close" aria-label="Close" title="' + lang.dialogBox.close + '">' +
                        this.icons.cancel +
                    '</button>' +
                    '<span class="ke-modal-title">' + lang.dialogBox.audioBox.title + '</span>' +
                '</div>' +
                    ...
                    html += '' +
                        '<div class="ke-dialog-form">' +
                            ...
                            '<div class="ke-dialog-form-files">' +
                                '<input class="ke-input-form _ke_audio_files" type="file" accept="audio/"' + option.audioAccept + (option.audioMultipleFile ? ' multiple="multiple"' : '') + '/>' +
    ...
    submitAction: function (fileList) {
        ...
        if (limitSize > 0) {
            ...
            if ((fileSize + infoSize) > limitSize) {
                this.closeLoading();
                const err = '[KothingEditor.audioUpload.fail] Size of uploadable total audios: ' + (limitSize/1000) + 'KB';
