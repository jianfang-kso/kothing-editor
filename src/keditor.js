...
export default {
    ...
    /**
     * @description Create the keditor
    ...
    create: function (idOrElement, options, _init_options) {
        ...
        if (!element) {
            if (typeof idOrElement === 'string') {
                throw Error('[KEDITOR.create.fail] The element for that id was not found (ID:"' + idOrElement + '")');
            }
        ...
        if (cons.constructed._top.id && document.getElementById(cons.constructed._top.id)) {
            throw Error('[KEDITOR.create.fail] The ID of the keditor you are trying to create already exists (ID:"' + cons.constructed._top.id + '")');
        }
