...
var _default = {
  ...
  setDialog: function setDialog() {
    ...
    var html = '' + '<div class="ke-dialog-header">' + '<button type="button" data-command="close" class="ke-btn ke-dialog-close" class="close" aria-label="Close" title="' + lang.dialogBox.close + '">' + this.icons.cancel + '</button>' + '<span class="ke-modal-title">' + lang.dialogBox.imageBox.title + '</span>' + '</div>' + '<div class="ke-dialog-tabs">' + '<button type="button" class="_ke_tab_link active" data-tab-link="image">' + lang.toolbar.image + '</button>' + '<button type="button" class="_ke_tab_link" data-tab-link="url">' + lang.toolbar.link + '</button>' + '</div>' + '<form method="post" enctype="multipart/form-data">' + '<div class="_ke_tab_content _ke_tab_content_image">' + '<div class="ke-dialog-body"><div style="border-bottom: 1px dashed #ccc;">';

    if (option.imageFileInput) {
      html += '' + '<div class="ke-dialog-form">' + '<label>' + lang.dialogBox.imageBox.file + '</label>' + '<div class="ke-dialog-form-files">' + '<input class="ke-input-form _ke_image_file" type="file" accept="image/"' + option.imageAccept + (option.imageMultipleFile ? ' multiple="multiple"' : '') + '/>' + '<button type="button" class="ke-btn ke-dialog-files-edge-button ke-file-remove" title="' + lang.controller.remove + '">' + this.icons.cancel + '</button>' + '</div>' + '</div>';
    }

    if (option.imageUrlInput) {
      html += '' + '<div class="ke-dialog-form">' + '<label>' + lang.dialogBox.imageBox.url + '</label>' + '<div class="ke-dialog-form-files">' + '<input class="ke-input-form ke-input-url" type="text" />' + (option.imageGalleryUrl && this.plugins.imageGallery ? '<button type="button" class="ke-btn ke-dialog-files-edge-button __ke__gallery" title="' + lang.toolbar.imageGallery + '">' + this.icons.image_gallery + '</button>' : '') + '</div>' + '<pre class="ke-link-preview"></pre>' + '</div>';
    }
    ...
    if (option.imageResizing) {
      ...
      html += '' + '<input class="ke-input-control _ke_image_size_x" placeholder="auto"' + (onlyPercentage ? ' type="number" min="1"' : 'type="text"') + (onlyPercentage ? ' max="100"' : '') + ' />' + '<label class="ke-dialog-size-x"' + heightDisplay + '>' + (onlyPercentage ? '%' : 'x') + '</label>' + '<input type="text" class="ke-input-control _ke_image_size_y" placeholder="auto"' + onlyPercentDisplay + (onlyPercentage ? ' max="100"' : '') + heightDisplay + '/>' + '<label' + onlyPercentDisplay + heightDisplay + '><input type="checkbox" class="ke-dialog-btn-check _ke_image_check_proportion" checked/>&nbsp;' + lang.dialogBox.proportion + '</label>' + '<button type="button" title="' + lang.dialogBox.revertButton + '" class="ke-btn ke-dialog-btn-revert" style="float: right;">' + this.icons.revert + '</button>' + '</div>';
    }

    html += '' + '<div class="ke-dialog-form ke-dialog-form-footer">' + '<label><input type="checkbox" class="ke-dialog-btn-check _ke_image_check_caption" />&nbsp;' + lang.dialogBox.caption + '</label>' + '</div>' + '</div>' + '</div>' + '<div class="_ke_tab_content _ke_tab_content_url" style="display: none">' + '<div class="ke-dialog-body">' + '<div class="ke-dialog-form">' + '<label>' + lang.dialogBox.linkBox.url + '</label><input class="ke-input-form _ke_image_link" type="text" />' + '<pre class="ke-link-preview"></pre>' + '</div>' + '<label><input type="checkbox" class="_ke_image_link_check"/>&nbsp;' + lang.dialogBox.linkBox.newWindowCheck + '</label>' + '</div>' + '</div>' + '<div class="ke-dialog-footer">' + '<div>' + '<label><input type="radio" name="kothing-editor_image_radio" class="ke-dialog-btn-radio" value="none" checked>' + lang.dialogBox.basic + '</label>' + '<label><input type="radio" name="kothing-editor_image_radio" class="ke-dialog-btn-radio" value="left">' + lang.dialogBox.left + '</label>' + '<label><input type="radio" name="kothing-editor_image_radio" class="ke-dialog-btn-radio" value="center">' + lang.dialogBox.center + '</label>' + '<label><input type="radio" name="kothing-editor_image_radio" class="ke-dialog-btn-radio" value="right">' + lang.dialogBox.right + '</label>' + '</div>' + '<button type="submit" class="ke-btn-primary" title="' + lang.dialogBox.submitButton + '"><span>' + lang.dialogBox.submitButton + '</span></button>' + '</div>' + '</form>';
  ...
  openTab: function openTab(e) {
    ...
    var i; // Get all elements with class="tabcontent" and hide them

    var tabContent = modal.getElementsByClassName(contentClassName);
    ...
    var tabLinks = modal.getElementsByClassName('_ke_tab_link');
  ...
  setup_reader: function setup_reader(files, imgLinkValue, newWindowCheck, width, height, align, filesLen, isUpdate) {
    ...
    try {
      var FileReader = this._w.FileReader;
