...
var _default = {
  ...
  setDialog: function setDialog() {
    ...
    if (option.audioFileInput) {
      html += '' + '<div class="ke-dialog-form">' + '<label>' + lang.dialogBox.audioBox.file + '</label>' + '<div class="ke-dialog-form-files">' + '<input class="ke-input-form _ke_audio_files" type="file" accept="audio/"' + option.audioAccept + (option.audioMultipleFile ? ' multiple="multiple"' : '') + '/>' + '<button type="button" data-command="filesRemove" class="ke-btn ke-dialog-files-edge-button ke-file-remove" title="' + lang.controller.remove + '">' + this.icons.cancel + '</button>' + '</div>' + '</div>';
    }
