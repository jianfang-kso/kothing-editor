...
function _default(context, pluginCallButtons, plugins, lang, options, _responsiveButtons) {
  ...
  var core = {
    ...
    getRange: function getRange() {
      ...
      if (selection.rangeCount > 0) {
        ...
        var rightDir = compareValue.ancestor && (compareValue.result === 0 ? so <= eo : compareValue.result > 1);
    ...
    getSelectionNode: function getSelectionNode() {
      if (util.isWysiwygDiv(this._variable._selectionNode)) {
        this._editorRange();
      }
    ...
    removeNode: function removeNode() {
      if (!this._resetRangeToTextNode()) {
        console.warn('[KothingEditor.core.removeNode.exception] An exception occurred while resetting the "Range" object.');
      }
          ...
          while (!util.isFormatElement(sNode) && !util.isWysiwygDiv(sNode)) {
            for (var _i5 = 0; _i5 < checkAttrs.length; _i5++) {
              if (sNode.nodeType === 1) {
                ...
                var classReg = /^\./.test(s) ? new RegExp("\\s*" + s.replace(/^\./, "") + "(\\s+|$)", "ig") : false;
    ...
    _nodeChange_oneLine: function _nodeChange_oneLine(element, newInnerNode, validation, startCon, startOff, endCon, endOff, isRemoveFormat, isRemoveNode, collapsed, _removeCheck, _getMaintainedNode, _isMaintainedNode) {
      ...
      function checkCss(vNode) {
        var regExp = new RegExp("(?:;|^|\\s)(?:" + cssText + "null)\\s*:[^;]*\\s*(?:;|$)", "ig");
        var style = "";
    ...
    _nodeChange_endLine: function _nodeChange_endLine(element, newInnerNode, validation, endCon, endOff, isRemoveFormat, isRemoveNode, _removeCheck, _getMaintainedNode, _isMaintainedNode) {
      ...
      (function recursionFunc(current, ancestor) {
        ...
        for (var i = childNodes.length - 1, vNode; i >= 0; i--) {
          var child = childNodes[i];
    ...
    _setCodeDataToEditor: function _setCodeDataToEditor() {
      ...
      if (options.fullPage) {
        ...
        for (var _i17 = 0, _len7 = attrs.length; _i17 < _len7; _i17++) {
          if (attrs[_i17].name === "contenteditable") {
            continue;
          }
      ...
      html = this._deleteDisallowedTags(html).replace(/(<[a-zA-Z0-9]+)[^>]*(?=>)/g, function (m, t) {
        ...
        if (v) {
          for (var i = 0, len = v.length; i < len; i++) {
            if (/^class="(?!(__ke__|ke-|katex))/.test(v[i])) {
              continue;
            }
    ...
    convertHTMLForCodeView: function convertHTMLForCodeView(html) {
      var returnHTML = "";
      var RegExp = _w.RegExp;
      var brReg = new RegExp("^(BLOCKQUOTE|PRE|TABLE|THEAD|TBODY|TR|TH|TD|OL|UL|IMG|IFRAME|VIDEO|AUDIO|FIGURE|FIGCAPTION|HR|BR|CANVAS|SELECT)$", "i");
    ...
    _init: function _init(reload, _initHTML) {
      ...
      this._disallowedTextTagsRegExp = disallowTextTags.length === 0 ? null : new RegExp("(<\\/?)(" + disallowTextTags.join("|") + ")\\b\\s*(?:[^>^<]+)?\\s*(?=>)", "gi"); // set whitelist
      ...
      this._fileManager.pluginRegExp = new RegExp("^(" + (filePluginRegExp.length === 0 ? "undefined" : filePluginRegExp.join("|")) + ")$", "i"); // cache editor's element
    ...
    _setDefaultFormat: function _setDefaultFormat(formatName) {
      ...
      if ((util.isRangeFormatElement(startCon) || util.isWysiwygDiv(startCon)) && util.isComponent(startCon.childNodes[range.startOffset])) {
        return;
      }
  ...
  var event = {
    ...
    onClick_wysiwyg: function onClick_wysiwyg(e) {
      ...
      if (typeof functions.onClick === "function") {
        functions.onClick(e, core);
      }
    },
    _balloonDelay: null,
    ...
    onInput_wysiwyg: function onInput_wysiwyg(e) {
      ...
      if (typeof functions.onInput === "function") {
        functions.onInput(e, core);
      }
    },
    ...
    onKeyDown_wysiwyg: function onKeyDown_wysiwyg(e) {
      ...
      var rangeEl = util.getRangeFormatElement(formatEl, null); // nested list
    ...
    onKeyUp_wysiwyg: function onKeyUp_wysiwyg(e) {
      ...
      if (typeof functions.onKeyUp === "function") {
        functions.onKeyUp(e, core);
      }
    },
    ...
    _setResponsiveToolbar: function _setResponsiveToolbar() {
      ...
      var sizeArray = event._responsiveButtonSize = ["default"];
  ...
  var functions = {
    ...
    setToolbarButtons: function setToolbarButtons(buttonList) {
      ...
      var newToolbar = _constructor["default"]._createToolBar(_d, buttonList, core.plugins, options);
    ...
    destroy: function destroy() {
      ...
      for (var _k in event) {
        if (util.hasOwn(event, _k)) {
          delete event[_k];
        }
      }

      for (var _k2 in context) {
        if (util.hasOwn(context, _k2)) {
          delete context[_k2];
        }
      }

      for (var _k3 in pluginCallButtons) {
        if (util.hasOwn(pluginCallButtons, _k3)) {
          delete pluginCallButtons[_k3];
        }
      }
      /** remove user object */
