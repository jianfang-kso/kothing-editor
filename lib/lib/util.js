...
var util = {
  ...
  getIncludePath: function getIncludePath(nameArray, extension) {
    ...
    if (!path) {
      throw Error('[KothingEditor.util.getIncludePath.fail] The KothingEditor installation path could not be automatically detected. (name: +' + path + ', extension: ' + extension + ')');
    }
  ...
  _setIframeCssTags: function _setIframeCssTags(options) {
    ...
    for (var f = 0, len = linkNames.length, path; f < len; f++) {
      ...
      if (!path || path.length === 0) {
        throw Error('[KothingEditor.constructor.iframe.fail] The kothingEditor CSS files installation path could not be automatically detected. Please set the option property "iframeCSSFileName" before creating editor instances.');
      }
