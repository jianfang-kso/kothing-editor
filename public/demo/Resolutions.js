export default {
  ...
  add: function (core, targetElement) {
    ...
    /** add event listeners */
    listDiv.querySelector('.ke-btn-primary').addEventListener('click', this.onClick.bind(core));
    listDiv.querySelector('.ke-btn').addEventListener('click', this.onClickRemove.bind(core));
  ...
  setSubmenu: function () {
    ...
    // @Required
    // A "ke-submenu" class is required for the top level element.
    listDiv.className = 'ke-submenu ke-list-layer';
    listDiv.innerHTML = '' +
            '<div class="ke-list-inner">' +
                '<ul class="ke-list-basic" style="width: 230px;">' +
                    '<li>' +
                        '<div class="ke-submenu-form-group">' +
                            '<input class="ke-input-form" type="text" placeholder="insert text" style="border: 1px solid #CCC;" />' +
                            '<button type="button" class="ke-btn-primary ke-tooltip">' +
                                '<strong>OK</strong>' +
                                '<span class="ke-tooltip-inner">' +
                                    '<span class="ke-tooltip-text">Append span</span>' +
                                '</span>' +
                            '</button>' +
                            '<button type="button" class="ke-btn ke-tooltip">' +
                                '<strong>X</strong>' +
                                '<span class="ke-tooltip-inner">' +
                                    '<span class="ke-tooltip-text">Remove</span>' +
                                '</span>' +
                            '</button>' +
                        '</div>' +
                    '</li>' +
  ...
  active: function (element) {
    ...
    } else if (this.util.hasClass(element, 'ke-custom-tag')) {
  ...
  onClick: function () {
    ...
    if (span) {
      ...
      this.functions.insertHTML('<span class="ke-custom-tag">' + value + '</span>');
      this.context.customSubmenu.textElement.value = '';
    }
